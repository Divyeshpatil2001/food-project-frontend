...
//     const order_id_data = order_save.data.order_id
//     console.log(order_save.data) 
//     console.log(cart)
//     console.log(userId)
...
//       "currency" : "INR"
//     })
//     console.log(razorPayment.data.data)
...
//         "order_id" : order_id,
//         "signature" : signature,
//         "amount" : getTotalAmount(order_id)
//       })
...
function AddToCart() {
  ...
  const handleCheckout = async() => {
    ...
    redirectToRazorpay(razorPayment.data.data.id);
  }

  const redirectToRazorpay = async (orderId) => {
    const options = {
      key: "rzp_test_05CcPoy7xdnDDa", 
      name: "FoodCrafters",
      description: "Test Transaction",
      ...
      handler: async function (response) {
        ...
        await completeOrder(razorpay_payment_id, razorpay_order_id, razorpay_signature);
      },
      prefill: {
        name: "Divyesh patil",
        email: "divyeshpatil@gmail.com",
  ...
  const completeOrder = async (payment_id, order_id, signature) => {
    try {
      const response = await axiosInstance.post('razorpay/order/complete/', {
        "payment_id" : payment_id,
        "order_id" : order_id,
